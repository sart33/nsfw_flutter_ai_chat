...
> **Internet required.** Chat and image generation run through the DeepSeek and WaveSpeed APIs. This is by design, not an optional online feature — the app cannot work offline. There are no developer-operated servers: requests go straight from your device to the providers.
...
No accounts. No cloud storage. No analytics or telemetry. Characters, chats, images, settings and API keys stay on the device — SQLite for data, app documents directory for images, `flutter_secure_storage` for keys. Requests go directly to DeepSeek and WaveSpeed; the developer has no access to any of it.
...
|                    | Required | Cost                          |
| ------------------ | -------- |-------------------------------|
| DeepSeek (chat)    | yes      | ~$0.03 per hour of active use |
| WaveSpeed (images) | optional | ~$0.005 per image             |

WaveSpeed minimum top-up: **$5**.
...
On first launch you are taken to the API Keys screen. Enter your DeepSeek key to start chatting; add a WaveSpeed key only if you want images. Setup walkthrough: [User Guide → API keys](https://sart33.github.io/uncensored-souls/user-guide.html#api-keys)
...
Get your chat API key at [platform.deepseek.com](https://platform.deepseek.com/api_keys).
Get your image-generation API key from [wavespeed.ai](https://wavespeed.ai/accesskey).
...
v1.2.5 (Android, Windows) · v1.2.0 (macOS) · in active development
